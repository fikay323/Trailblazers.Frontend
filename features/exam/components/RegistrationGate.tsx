...
export function RegistrationGate() {
	...
	return (
		<Card className="w-full max-w-3xl border-slate-800 bg-slate-900/70 backdrop-blur-md shadow-2xl mx-auto text-slate-100">
			...
			<div className="bg-slate-950/80 border-b border-slate-800 px-3.5 py-3 sm:px-6 sm:py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 sm:gap-3 rounded-t-xl">
				<div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
					<div className="h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-orange-600/20 text-orange-400 flex items-center justify-center font-bold text-xs sm:text-sm border border-orange-500/30 shrink-0">
						{user.fullName?.charAt(0).toUpperCase() || 'S'}
					</div>
					<div className="min-w-0">
						<div className="text-xs sm:text-sm font-bold text-white flex flex-wrap items-center gap-1.5">
							<span className="truncate">{user.fullName}</span>
							<span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-950 text-emerald-300 border border-emerald-800 shrink-0">
								<CheckCircle2 className="h-3 w-3 mr-1" />
								Active Candidate
							</span>
						</div>
						<div className="text-[11px] sm:text-xs text-slate-400 font-mono truncate">{user.email}</div>
					</div>
				</div>

				<Link href="/student/dashboard" className="text-xs text-orange-400 hover:text-orange-300 flex items-center gap-1 shrink-0 self-start sm:self-auto">
					Dashboard History
					<ChevronRight className="h-3.5 w-3.5" />
				</Link>
			</div>

			<CardContent className="p-3.5 sm:p-8 space-y-4 sm:space-y-6">
				{error && (
					<div className="p-3 sm:p-4 rounded-lg bg-red-950/60 border border-red-800 text-red-300 text-xs flex items-center gap-2">
						<AlertCircle className="h-4 w-4 shrink-0 text-red-400" />
						<span>{error}</span>
					</div>
				)}

				<form onSubmit={handleLaunchExam} className="space-y-4 sm:space-y-6">
					...
					<div className="space-y-1.5 sm:space-y-2">
						...
						<select
							...
							className="w-full h-10 sm:h-11 rounded-lg border border-slate-800 bg-slate-950 px-3 text-xs sm:text-sm text-white focus:outline-none focus:ring-1 focus:ring-orange-500 cursor-pointer font-semibold"
						>
							{metadata.years.map((y) => (
								<option key={y} value={y}>
									{y} JAMB UTME Past Questions
								</option>
							))}
						</select>
					</div>
					...
					{/* Subject Selection */}
					<div className="space-y-2.5 sm:space-y-3 pt-2 border-t border-slate-800/80">
						...
						<div className="p-2.5 sm:p-3 rounded-lg border border-emerald-900/60 bg-emerald-950/20 flex items-center justify-between gap-2 text-xs">
							<div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
								<CheckCircle2 className="h-4 w-4 text-emerald-400 shrink-0" />
								<span className="font-bold text-white truncate">Use of English</span>
								<span className="text-[11px] text-emerald-300/80 font-medium hidden sm:inline">(Compulsory for all candidates)</span>
							</div>
							<div className="flex items-center gap-1.5 shrink-0">
								<span className="text-[10px] text-emerald-300/80 font-medium sm:hidden">Compulsory</span>
							...
							{electiveSubjects.map((subject) => {
								...
								return (
									<button
										...
										className={`p-2.5 sm:p-3 rounded-lg text-xs font-semibold border text-left transition-all cursor-pointer flex items-center justify-between gap-1.5 ${isSelected
					...
					<div className="p-3 sm:p-4 rounded-lg bg-slate-950/80 border border-slate-800 text-xs text-slate-300 space-y-1.5 sm:space-y-2">
					...
					<div className="pt-1 sm:pt-2">
						<Button
							...
							className="w-full h-11 sm:h-12 text-sm font-bold bg-orange-600 hover:bg-orange-700 text-white shadow-lg shadow-orange-950/50 transition-all cursor-pointer flex items-center justify-center gap-2"
						>
							{isLoading ? (
								<>
